...
//----------------------------Method to get access period as formatted string-------------------------------------------
    ...
    public int switchIndex (int currentIndexNo, int targetIndexNo, boolean switchInWaitList)
    {
    ...
    public boolean checkIfEnrolledInAnyCourse ()
    {
        if (currentLogOnUser instanceof Student)
        {
            if (((Student) currentLogOnUser).getCourseIndexList().size() > 0)
                return true;
            else
                return false;
        }
    ...
    public int student_EnrolIndex (int indexNo)
    {
        ...
        {
            //TODO: Add a warning here for users to proceed with switching index. Stops this code if you don't want users to switch
            int switchResult = switchIndex(indexInSameCourse.getIndexNum(), indexToJoin.getIndexNum(), switchInWaitList);
            switch (switchResult)
            {
                case 1:
                    saveData();
    ...
    public void student_SwitchIndex (int indexNo)
    {
